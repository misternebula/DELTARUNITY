...
namespace Assets
{
	internal class GamemakerCamera : MonoBehaviour
	{
		...
		private static void RenderText(GMTextJob textJob)
		{
			...
			var totalWidth = 0;
			for (var i = 0; i < textJob.text.Length; i++)
			{
				var asciiIndex = (int)textJob.text[i];
				var entry = textJob.asset.entriesDict[asciiIndex];

				if (i == textJob.text.Length - 1)
				{
					totalWidth += entry.texture.width;
				}
				else
				{
			...
			switch (textJob.halign)
			{
				case HAlign.left:
					pos = new Vector3(textJob.screenPos.x, -textJob.screenPos.y, 0);
					break;
				case HAlign.center:
					pos = new Vector3(textJob.screenPos.x - (totalWidth / 2), -textJob.screenPos.y, 0);
					break;
				case HAlign.right:
					pos = new Vector3(textJob.screenPos.x - totalWidth, -textJob.screenPos.y, 0);
					break;
			}
			...
			foreach (var character in textJob.text)
			{
				var asciiIndex = (int)character;
				var entry = textJob.asset.entriesDict[asciiIndex];

				var material = Instance.SpriteMaterial;

				if (textJob.isColor)
				{
					material = Instantiate(SpriteManager.SpriteManager.Instance.TextColorMaterial);

					// todo : optimise?
					material.SetColor("_VertOneColor", textJob.c1);
					material.SetColor("_VertTwoColor", textJob.c2);
					material.SetColor("_VertThreeColor", textJob.c3);
					material.SetColor("_VertFourColor", textJob.c4);
				}

				Graphics.DrawTexture(
					new Rect(advancement + entry.offset, 0, entry.texture.width, -entry.texture.height),
					entry.texture,
					new Rect(0, 0, 1, 1),
					0, 0, 0, 0,
					new Color(textJob.blend.r, textJob.blend.g, textJob.blend.b, (float)textJob.alpha),
